---
layout: post
title: 'Kubernetes: курс молодого бойца'
permalink: /k8s-101/
tags:
- kubernetes
- docker
...
Долго не мучая воображение, я нашел простенький [пример](https://github.com/paulbouwer/hello-kubernetes/blob/master/yaml/hello-kubernetes.yaml) на Гитхабе. Предлагаю открыть его и пробежаться по нему глазами, сильно не углубляясь в происходящее, а после этого -- запустить его:
```
...
Не сложно догадаться, что если бы я отредактировал этот ямлик и вновь скормил бы его куберу -- он бы заметил эти изменения и отчитался бы о тех ресурсах, что он обновит в соответствии с ними. В целом ситуация похожа на работу `docker-compose`.
...
Отлично, работает, мы все сделали правильно, но... кубер сказал нам, что создал всего два ресурса, а в выхлопе контента сильно больше -- давайте разбираться. Итак, в YAML-файле у нас было определено два ресурса, сервис и деплоймент, оба с именем `hello-kubernetes`, мы их видем в списке, к ним претензий нет. Сразу отбросим `service/kubernetes` -- это служебный сервис для общения приложений с API кубера, присутствует всегда в каждом неймспейсе.
...
Пока прикольно, продолжаем баловаться: что случится, если мы удалим под, симулировав падение ноды, где он был запущен:
```
...
````
Вместо пода `*-5nxv7` был создан `*-x58hh`, мы видим это по возрасту подов. Мы застрахованы от падения нод и еще целого класса неприятностей.
...
Момент, который стоит подметить: сумма подов в старом и новом репликасетах никогда не падает ниже `DESIRED` в деплойменте: 4+1, 3+2, 2+3, 1+4, 0+4. Это означает, что во время релиза у нас не будет просадки в "лошадиных силах", обслуживающих пользователей.
...
В нашем случае 25% от 4 реплик - это ровно 1, и мы не увидели разницы... но в случае какого-нибудь особо отмасштабированного сервиса с 50 репликами поштучный релиз мог бы затянуться на несколько часов.
